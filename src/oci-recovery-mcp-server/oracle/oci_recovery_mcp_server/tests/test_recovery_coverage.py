...
def test_server_helpers_cover_serialization_config_and_wrapping(monkeypatch, tmp_path):
    ...
    # thing this server still translates is the legacy unseparated "apikey".
    monkeypatch.setenv("ORACLE_MCP_AUTH_METHOD", "apikey")
    assert server._legacy_auth_type_override() is server.AuthType.API_KEY
    for shared_spelling in ("api-key", "api_key", "session"):
        monkeypatch.setenv("ORACLE_MCP_AUTH_METHOD", shared_spelling)
        assert server._legacy_auth_type_override() is None
...
def test_profile_auth_context_defers_to_the_shared_library(monkeypatch):
    ...
    # Only the legacy unseparated spelling is translated.
    monkeypatch.setenv("ORACLE_MCP_AUTH_METHOD", "apikey")
...
def test_client_factories_cover_profile_and_http_auth_paths(monkeypatch):
    monkeypatch.setattr(
        server,
        "_wrap_oci_client",
