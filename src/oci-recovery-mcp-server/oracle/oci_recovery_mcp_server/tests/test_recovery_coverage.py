...
Branch, fallback, and error-path coverage for the model mappers, client
factories, compartment helpers, and tool wrappers.
"""
...
class TestRestoredClientFactoryCoverage:
    ...
    def test_oauth_client_uses_request_scoped_token_exchange_signer(
        self,
        _mock_auth_method,
        ...
        mock_current_tenancy.return_value = SimpleNamespace(
            alias="acme",
        ...
        # A fresh signer per call: OAuth-mode UPST signers are never cached.
...
def test_profile_auth_context_selects_auth_type_per_method(monkeypatch):
    captured = []
    ...
    assert [options.auth_type for options in captured] == [
        server.AuthType.API_KEY,
        server.AuthType.SECURITY_TOKEN,
    ]
    assert {options.profile_name for options in captured} == {"PROFILE1"}


def test_client_factories_cover_profile_and_oauth_auth_paths(monkeypatch):
    monkeypatch.setattr(
        server,
        "_wrap_oci_client",
    ...
    # oauth: minimal regional config + per-request token-exchange signer.
    oauth_signer = object()
    ...
    monkeypatch.setattr(
        ...
        lambda: SimpleNamespace(
            alias="acme", tenancy_id="ocid1.tenancy.oc1..t", region="us-ashburn-1"
        ),
    )
    monkeypatch.setattr(
    ...
    assert recovery_client.call_args.kwargs["signer"] is oauth_signer
    assert database_client.call_args.kwargs["signer"] is oauth_signer
...
def test_new_client_factories_cover_limits_work_requests_and_subscription(monkeypatch):
    ...
    assert work_request_client.call_args.kwargs["signer"] is oauth_signer
    assert limits_client.call_args.kwargs["signer"] is oauth_signer
    assert subscribed_service_client.call_args.kwargs["signer"] is oauth_signer
...
def test_logging_tool_wrapper_tenancy_and_apikey_client_paths(monkeypatch, tmp_path):
    ...
    # oauth: the tenancy is bound to the verified token's tenancy claim, and the
    # local profile/override is ignored entirely.
    monkeypatch.setenv("TENANCY_ID_OVERRIDE", "ignored-override")
    monkeypatch.setattr(server, "_effective_auth_method", lambda: "oauth")
    monkeypatch.setattr(
        server,
        "_current_tenancy",
        lambda: SimpleNamespace(
            alias="acme", tenancy_id="ocid1.tenancy.oc1..claim", region="us-ashburn-1"
        ),
    )
    assert server.get_tenancy() == "ocid1.tenancy.oc1..claim"
